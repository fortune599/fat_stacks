...
def draw_polygons( points, screen, color ):
    ...
    while point < len(points) - 2:
        Ax = int(points[point+1][0] - points[point][0])
        Bx = int(points[point+2][0] - points[point][0])
...
def add_box( points, x, y, z, width, height, depth ):
    ...
    add_polygon(points, x1, y1, z, x1, y, z1, x1, y1, z1)
    ...
    for lat in range(lat_start, lat_stop):
        for longt in range(longt_start, longt_stop+1):
            ...
            add_polygon(edges,
                        ...
                        points[(index + num_steps) % (lat_stop * num_steps)][1],
                        points[(index + num_steps) % (lat_stop * num_steps)][2],
                        points[(index + 1 + num_steps) % (lat_stop * num_steps)][0],
                        points[(index + 1 + num_steps) % (lat_stop * num_steps)][1],
                        points[(index + 1 + num_steps) % (lat_stop * num_steps)][2])
    ...
    for lat in range(lat_start, lat_stop):
        for longt in range(longt_start, longt_stop):
            ...
            add_polygon(edges,
                        ...
                        points[index][1],
                        points[index][2],
                        points[(index + 1) % (longt_stop * num_steps)][0],
                        points[(index + 1) % (longt_stop * num_steps)][1],
                        points[(index + 1) % (longt_stop * num_steps)][2],
                        points[(index + num_steps) % (lat_stop * num_steps)][0],
                        points[(index + num_steps) % (lat_stop * num_steps)][1],
                        points[(index + num_steps) % (lat_stop * num_steps)][2])
            add_polygon(edges,
